...
$(document).ready(function() {
	...
	var Worklist = function (index) {
		...
		self.init = function(index) {
			...
			while(self.search().backlog().length) {	
				course = self.search().backlog()[0];				
				if(course.status !== "Registered") self.addCourse(course);
				else self.search().removeFromBacklog(course);			
			}

			var registeredCourses  = $.map(window.registeredCourses, function(course, index) { return new Course(course); });
			for(var i = 0; i < registeredCourses.length; i++) {
				self.paintOver(registeredCourses[i]);
			}
